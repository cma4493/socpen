...
function CurrentProjectID() {
	...
	return "{AC00512B-B959-4ABC-B03E-21192746C63D}";
}
...
class cBreadcrumb {
	...
	// Render
	function Render() {
		$nav = "<ul class=\"breadcrumb\"><li><i class='icon-home home-icon'></i><a href='index.php'>Home</a></li>";
		if (is_array($this->Links)) {
			$cnt = count($this->Links);
			for ($i = 0; $i < $cnt; $i++) {
	...
	function RenderButtonGroup($body) {
		...
		if (preg_match_all('/<a([^>]*)>([\s\S]*?)<\/a\s*>/i', $body, $matches, PREG_SET_ORDER)) {
			foreach ($matches as $match) {
				...
				$caption = $match[2];
				ew_PrependClass($class, ''); // Prepend button classes
				if ($this->ButtonClass <> "")
		...
		if ($links <> "")
			$btngroup = '<div class="btn-group">' . $links . '</div>';
		else
			$btngroup = "";
		foreach ($btns as $btn)
			$btngroup .= $btn;
...
class cAdvancedSecurity {
			...
			if ($rs = $conn->Execute($sSql)) {
				if (!$rs->EOF) {
					...
					if ($ValidateUser) {
						...
						$this->setSessionUserID($rs->fields('uid')); // Load User ID
						$this->setSessionParentUserID($rs->fields('uid')); // Load parent User ID
	...
	function LoadUserID() {
		...
		if (strval($this->CurrentUserID) == "") {
			...
			$sFilter = $tbl_user->UserIDFilter($this->CurrentUserID);
			$sSql = $tbl_user->GetSQL($sFilter, '');
			if ($RsUser = $conn->Execute($sSql)) {
				while (!$RsUser->EOF) {
					$this->AddUserID($RsUser->fields('uid'));
					$RsUser->MoveNext();
				}
				$RsUser->Close();
			}
...
// jfsbaldo
function ew_WriteUpdates($pfx, $dt, $usr, $table, $field, $keyvalue, $dateUpdated, $UpdateRemarks, $oldvalue, $newvalue) {
	$usrwrk = $usr;
	if ($usrwrk == "") $usrwrk = "-1"; // Assume Administrator if no user
	if (EW_AUDIT_TRAIL_TO_DATABASE) {
		global $conn;
		$sAuditSql = "INSERT INTO tbl_updates (`CreatedDate`, `Createdby`, `field`, `PensionerID`, `old_value`, `new_value`, `DateUpdated`, `Remarks`) VALUES ('".$dt."', '".$usrwrk."', '".$field."', '".$keyvalue."','".$oldvalue."', '".$newvalue."','".$dateUpdated."','".$UpdateRemarks."')";
		$conn->Execute($sAuditSql);
	} else {
		$sTab = "\t";
		$sHeader = "date/time" . $sTab . "user" . $sTab .
			"field" . $sTab . "key value" . $sTab . "old value" . $sTab . "new value";
		$sMsg = $dt . $sTab . $usrwrk . $sTab .
			$field . $sTab . $keyvalue . $sTab . $oldvalue . $sTab . $newvalue;
		$sFolder = EW_AUDIT_TRAIL_PATH;
		$sFn = $pfx . "_" . date("Ymd") . ".txt";
		$filename = ew_UploadPathEx(TRUE, $sFolder) . $sFn;
		if (file_exists($filename)) {
