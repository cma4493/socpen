...
$lib_physical_condition_edit = NULL; // Initialize page object first

class clib_physical_condition_edit extends clib_physical_condition {
	...
	// Page object name
	var $PageObjName = 'lib_physical_condition_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->physconditionID->CurrentValue == "")
			$this->Page_Terminate("lib_physical_conditionlist.php"); // Invalid key, return to list
		...
		switch ($this->CurrentAction) {
			case "I": // Get a record to display
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("lib_physical_conditionlist.php"); // No matching record, return to list
				}
				break;
			Case "U": // Update
	...
	function LoadFormValues() {
		...
		if (!$this->physconditionName->FldIsDetailKey) {
			$this->physconditionName->setFormValue($objForm->GetValue("x_physconditionName"));
		}
	...
	function RestoreFormValues() {
		...
		$this->physconditionName->CurrentValue = $this->physconditionName->FormValue;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$this->physconditionID->EditValue = $this->physconditionID->CurrentValue;
			$this->physconditionID->ViewCustomAttributes = "";

			// physconditionName
			$this->physconditionName->EditCustomAttributes = "";
			$this->physconditionName->EditValue = ew_HtmlEncode($this->physconditionName->CurrentValue);
			$this->physconditionName->PlaceHolder = ew_HtmlEncode(ew_RemoveHtml($this->physconditionName->FldCaption()));
	...
	function ValidateForm() {
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if (!$this->physconditionName->FldIsDetailKey && !is_null($this->physconditionName->FormValue) && $this->physconditionName->FormValue == "") {
			ew_AddMessage($gsFormError, $Language->Phrase("EnterRequiredField") . " - " . $this->physconditionName->FldCaption());
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// physconditionName
			$this->physconditionName->SetDbValueDef($rsnew, $this->physconditionName->CurrentValue, "", $this->physconditionName->ReadOnly);

			// modified_by
			$this->modified_by->SetDbValueDef($rsnew, CurrentUserID(), NULL);
			$rsnew['modified_by'] = &$this->modified_by->DbValue;

			// date_modified
			$this->date_modified->SetDbValueDef($rsnew, ew_CurrentDateTime(), NULL);
			$rsnew['date_modified'] = &$this->date_modified->DbValue;
...
// Create page object
if (!isset($lib_physical_condition_edit)) $lib_physical_condition_edit = new clib_physical_condition_edit();

// Page init
$lib_physical_condition_edit->Page_Init();

// Page main
$lib_physical_condition_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$lib_physical_condition_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var lib_physical_condition_edit = new ew_Page("lib_physical_condition_edit");
lib_physical_condition_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = lib_physical_condition_edit.PageID; // For backward compatibility

// Form object
var flib_physical_conditionedit = new ew_Form("flib_physical_conditionedit");

// Validate form
flib_physical_conditionedit.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			elm = this.GetElements("x" + infix + "_physconditionName");
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($lib_physical_condition->physconditionName->FldCaption()) ?>");
...
flib_physical_conditionedit.Form_CustomValidate = 
...
flib_physical_conditionedit.ValidateRequired = true;
<?php } else { ?>
flib_physical_conditionedit.ValidateRequired = false; 
<?php } ?>
...
<?php $lib_physical_condition_edit->ShowPageHeader(); ?>
<?php
$lib_physical_condition_edit->ShowMessage();
?>
<form name="flib_physical_conditionedit" id="flib_physical_conditionedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="lib_physical_condition">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_lib_physical_conditionedit" class="table table-bordered table-striped">
...
<?php echo $lib_physical_condition->physconditionID->EditValue ?></span>
</span>
<input type="hidden" data-field="x_physconditionID" name="x_physconditionID" id="x_physconditionID" value="<?php echo ew_HtmlEncode($lib_physical_condition->physconditionID->CurrentValue) ?>">
<?php echo $lib_physical_condition->physconditionID->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($lib_physical_condition->physconditionName->Visible) { // physconditionName ?>
	<tr id="r_physconditionName">
		<td><span id="elh_lib_physical_condition_physconditionName"><?php echo $lib_physical_condition->physconditionName->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
...
<input type="text" data-field="x_physconditionName" name="x_physconditionName" id="x_physconditionName" size="30" maxlength="50" placeholder="<?php echo $lib_physical_condition->physconditionName->PlaceHolder ?>" value="<?php echo $lib_physical_condition->physconditionName->EditValue ?>"<?php echo $lib_physical_condition->physconditionName->EditAttributes() ?>>
</span>
<?php echo $lib_physical_condition->physconditionName->CustomMsg ?></td>
	</tr>
<?php } ?>
</table>
</td></tr></table>
...
<script type="text/javascript">
flib_physical_conditionedit.Init();
<?php if (EW_MOBILE_REFLOW && ew_IsMobile()) { ?>
ew_Reflow();
<?php } ?>
</script>
<?php
$lib_physical_condition_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$lib_physical_condition_edit->Page_Terminate();
?>
