...
ftbl_useradd.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($tbl_user->activated->FldCaption()) ?>");
			elm = this.GetElements("x" + infix + "_profile");
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($tbl_user->profile->FldCaption()) ?>");
...
ftbl_useradd.Lists["x_region_code"] = {"LinkField":"x_region_code","Ajax":true,"AutoFill":false,"DisplayFields":["x_region_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php if ($tbl_user->activated->Visible) { // activated ?>
	<tr id="r_activated">
		<td><span id="elh_tbl_user_activated"><?php echo $tbl_user->activated->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $tbl_user->activated->CellAttributes() ?>>
<span id="el_tbl_user_activated" class="control-group">
...
if (is_array($tbl_user->activated->EditValue)) {
	...
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($tbl_user->activated->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;
?>
<option value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?>>
<?php echo $arwrk[$rowcntwrk][1] ?>
...
<?php echo $tbl_user->activated->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($tbl_user->profile->Visible) { // profile ?>
	<tr id="r_profile">
		<td><span id="elh_tbl_user_profile"><?php echo $tbl_user->profile->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $tbl_user->profile->CellAttributes() ?>>
<span id="el_tbl_user_profile" class="control-group">
<textarea data-field="x_profile" name="x_profile" id="x_profile" cols="35" rows="4" placeholder="<?php echo $tbl_user->profile->PlaceHolder ?>"<?php echo $tbl_user->profile->EditAttributes() ?>><?php echo $tbl_user->profile->EditValue ?></textarea>
</span>
