	...
	<div>
		...
		<div>
			<select name="quarter" id="quarter">
				<option value="" selected>Please Select Quarter</option>
		...
		<div>
			<select name="paid_unpaid" id="paid_unpaid">
				<option value="" selected>Please Payment Status</option>
				<option value="0">Unpaid</option>
				<option value="1">Paid</option>
			</select>
		</div>
		<div>
			<button type="submit" class="btn btn-sm btn-success" onclick="return confirm('Are you sure?')" name="generatebtn" value="1">Generate</button>
		</div>
	</div>
</form>
<?php
if ($_REQUEST['generatebtn'] == 1){
	/*echo '<pre>';
	print_r($_REQUEST);
	echo '</pre>';*/
	$region = $_REQUEST['jb_region_code'];
	$province = $_REQUEST['jb_prov_code'];
	$municipality = $_REQUEST['jb_city_code'];
	$barangay = $_REQUEST['jb_brgy_code'];
