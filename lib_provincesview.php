...
$lib_provinces_view = NULL; // Initialize page object first

class clib_provinces_view extends clib_provinces {
	...
	// Page object name
	var $PageObjName = 'lib_provinces_view';
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["prov_code"] <> "") {
			$this->RecKey["prov_code"] = $_GET["prov_code"];
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["prov_code"] <> "") {
				...
				$sReturnUrl = "lib_provinceslist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "lib_provinceslist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "lib_provinceslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($lib_provinces_view)) $lib_provinces_view = new clib_provinces_view();

// Page init
$lib_provinces_view->Page_Init();

// Page main
$lib_provinces_view->Page_Main();
...
// Page object
var lib_provinces_view = new ew_Page("lib_provinces_view");
lib_provinces_view.PageID = "view"; // Page ID
var EW_PAGE_ID = lib_provinces_view.PageID; // For backward compatibility

// Form object
var flib_provincesview = new ew_Form("flib_provincesview");

// Form_CustomValidate event
flib_provincesview.Form_CustomValidate = 
...
flib_provincesview.ValidateRequired = true;
<?php } else { ?>
flib_provincesview.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$lib_provinces_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($lib_provinces_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
...
?>
<form name="flib_provincesview" id="flib_provincesview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="lib_provinces">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_lib_provincesview" class="table table-bordered table-striped">
<?php if ($lib_provinces->prov_code->Visible) { // prov_code ?>
	<tr id="r_prov_code">
		<td><span id="elh_lib_provinces_prov_code"><?php echo $lib_provinces->prov_code->FldCaption() ?></span></td>
...
<?php echo $lib_provinces->prov_code->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($lib_provinces->prov_name->Visible) { // prov_name ?>
	<tr id="r_prov_name">
		<td><span id="elh_lib_provinces_prov_name"><?php echo $lib_provinces->prov_name->FldCaption() ?></span></td>
...
<?php echo $lib_provinces->prov_name->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($lib_provinces->region_code->Visible) { // region_code ?>
	<tr id="r_region_code">
		<td><span id="elh_lib_provinces_region_code"><?php echo $lib_provinces->region_code->FldCaption() ?></span></td>
...
<?php echo $lib_provinces->region_code->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php
$lib_provinces_view->Page_Terminate();
?>
