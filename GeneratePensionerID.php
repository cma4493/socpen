...
class GeneratePensionerID extends DAO{
	function retListing4IDs($CurrentUserID_EW){
		...
		foreach($result as $i=>$rowData){
			$recordlist[$i] = array(
				...
				"PensionerID"=>$rowData['psgc_brgy'] . '-' . $CurrentUserID_EW . '-' . $rowData['SeniorID']
			);
		}
		return $recordlist;
	}
...
if (isset($_REQUEST['codegen'])) {
	$paramExplodeArray = explode("::",$_REQUEST['codegen']);
	$filename = $paramExplodeArray[0];
	$codeGen1 = $paramExplodeArray[1];
	$GeneratePensionerID = new GeneratePensionerID();
	$initData = $GeneratePensionerID->retListing4IDs(CurrentUserID());
	foreach($initData as $listData):
		// print_r($GeneratePensionerID->updatePensionerID($listData['SeniorID'],$listData['PensionerID']));
		$result = $GeneratePensionerID->updatePensionerID($listData['SeniorID'],$listData['PensionerID'],$codeGen1);
		$result += 1;
	endforeach;
	echo "IDs successfully generated!" . "<br>";
	echo "Download generated file" . "<br>";
	echo "<form id=\"downloadpensioner\" method=\"post\" action=\"downloadpensionerid.php\">";
	echo "<input type=\"hidden\" value=\"".$_REQUEST['codegen']."\" name=\"codegenids\">" . "<br>";
	echo "<input type=\"submit\" value=\"Download\">";
	echo "</form>";
} else {
	echo "Error generating IDs, please make sure you have uploaded pensioners<br>";
}
echo "</pre>";
?>
