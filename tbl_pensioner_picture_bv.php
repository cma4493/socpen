...
$tbl_pensioner_picture_blobview = NULL; // Initialize page object first

class ctbl_pensioner_picture_blobview extends ctbl_pensioner {
	...
	// Page object name
	var $PageObjName = 'tbl_pensioner_picture_blobview';
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'tbl_pensioner');
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('tbl_pensioner', 'x_picture');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				...
				if (trim(strval($this->Recordset->fields('picturetype'))) <> "") {
					header("Content-type: ". $this->Recordset->fields('picturetype'));
				} else {
					if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					 header("Content-type: " . ew_ContentType(substr($data, 0, 11)));
				}
				if (trim(strval($this->Recordset->fields('picturename'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('picturename') . "\"");
				}
...
// Create page object
if (!isset($tbl_pensioner_picture_blobview)) $tbl_pensioner_picture_blobview = new ctbl_pensioner_picture_blobview();

// Page init
$tbl_pensioner_picture_blobview->Page_Init();

// Page main
$tbl_pensioner_picture_blobview->Page_Main();
?>
<?php
$tbl_pensioner_picture_blobview->Page_Terminate();
?>
